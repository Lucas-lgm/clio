...
const COMMUNITY_SKILLS: SkillEntry[] = [
  ...
  {
    name: 'tdd',
    description: 'Test-driven development with red-green-refactor loop. Use when implementing any feature or bugfix, before writing implementation code.',
    keywords: 'tdd,test,testing,red-green-refactor,TDD,测试,测试驱动',
    content: [
      ...
      '',
      'No exceptions. Don\'t keep it as "reference". Don\'t "adapt" it while writing tests. Delete means delete. Implement fresh from tests.',
      '',
      '## Red-Green-Refactor',
      '',
      ...
      '',
      'Confirm the test fails (not errors), failure message is expected, and it fails because feature missing (not typos).',
      '',
      ...
      '**MANDATORY.**',
      '',
      'Confirm the test passes, other tests still pass, and output is pristine.',
      '',
      '### REFACTOR - Clean Up',
      '',
      ...
      '## Example: Bug Fix',
      '',
      'RED: Write `test(\'rejects empty email\')` → FAIL',
      'GREEN: Add `if (!email?.trim()) return { error: \'Email required\' }` → PASS',
      ...
      '## Final Rule',
      '',
      'Production code exists and failed first — otherwise it\'s not TDD. No exceptions without the user\'s permission.',
      '',
